...
We will also introduce a new difference simply called "Character difference" that counts the difference in states pattern (ranging from 0 - no difference - to 1 - complete difference) as follows:

![equation](http://www.sciweavers.org/tex2img.php?eq=1%20-%20%5Cleft%28%20%5Cfrac%7B%5Cmid%20%7B%5Cfrac%7B%5Cabs%7B%5Csum_%7Bi%7D%5E%7Bn%7D%28%5Cmid%20x_%7Bi%7D%20-%20y_%7Bi%7D%5Cmid%20%29%7D%7D%20%7Bn%7D-%5Cfrac%7B1%7D%7B2%7D%7D%20%5Cmid%20%20%7D%7B%5Cfrac%7B1%7D%7B2%7D%7D%5Cright%29&bc=White&fc=Black&im=jpg&fs=12&ff=arev&edit=0)
<!-- \begin{equation}
...
For example, a character `{8,8,2,1,3,0}` will be normalised by changing all the occurences of the first state (`8`) by `0`, then all the occurences of the second one (`2`) by `1` and so on resulting in the character being normalised as `{0,0,1,2,3,4}`.
...
Also, will will check if our difference metric is triangular (i.e. if `A` = `B` then the difference `(A,X)` must be equal to `(B,X)`).
...
In this case again, we expect `A` and 

Straight away, we expect `A` and `B` to be totally correlated/not different as well as well as `C` and `D`.

...
We've added the proportion of replaced characters in each matrices for each ``minimised'', ``maximised'' and ``randomised'' scenarios in the Supplementary material 3, figures @@@.
It is worth noting that there is a significant difference in terms of number of number of replaced characters in each scenarios (the ``minimised'' matrices have, on average, more characters replaced than the ``randomised'' that has in turns more characters replaced than the ``maximised'') but non in terms of number of taxa or characters.
...
%TG: add some text to the manuscript parphrasing this? It's kind of already written in there...

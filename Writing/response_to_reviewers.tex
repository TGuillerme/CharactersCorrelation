...
\begin{enumerate}
\item Distinctiveness: $CD_{(x,y)} = 0 \Leftrightarrow x = y$, proved in equation 10 (note that we corrected the error highlighted by reviewer 2 with a more restrictive definition of $x$ and $y$).
\item Non-negativity: $CD_{(x,y)} \geq 0$, proved in equation 9.
\item Symmetry: $CD_{x,y} = CD_{y,x}$, proved in equation 11.
\item Triangle inequality: $CD_{(x,y)} \leq CD_{(x,z)} + CD_{(z,y)}$, proved in equation 12.
\end{enumerate}

We did not found a proof for the Four-point condition that satisfied the equation:


$CD_{(i,j)} + CD_{(m,n)} \leq \text{max}{(CD_{(i,m)} + CD_{(j,n)} ; CD_{(j,m)} + CD_{(i,n)} )}$


So we cannot confirm or reject that $CD$ is a evolutionary distance metric.
...
%TG: Specify the Gower distance (comparable)
%TG: yes, n is fixed here (this is just for the CD definition)
%TG: no, characters with missing data should be more dissimilar: since the similarity is measured in terms of shared splits, a missing data can always involve a new split (if the ? is a new state)
%TG: and not sure about the directionality thing.

%TG: TODO: clarify the CD metric definition

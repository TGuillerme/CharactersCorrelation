...
In this study, use a metric of raw character difference as a proxy for character correlation.
...
%ap- will you confront the fact that this says that having character correlation is an advantage?
This means that dataset with low correlation between characters will make it more complicated to estimate a correct topology.
...
%ap but if cc increase recovering correct topology, then Bayesian is bad and we should use parsimony. Needs clarifying.
...
%MB: I think a light rewriting  of this opening paragraph would be good, focusing it more on a palaeontological audience (given the new target journal). We can cut the apologetics for morphology and focus it on palaeontology-relevant stuff  a bit more.
...
First, molecular and morphological data often differ in the size and number of states analysed.
 % (molecular matrices are generally larger with mainly four states - not including missing data or indels - and morphological matrices are smaller with generally two states but may have an arbitrary number of states (up to that supported by phylogenetic software; \citealt{Guillerme2016146}).
Second, correlations have been proposed to be stronger in morphological data
%(i.e. ``concentrated'' correlations)
but weaker and more ``spread-out'' in molecular data \citep[][]{huelsenbeck1999effect}. %i.e. ``diffused'' correlations - though some regions of the genome may be thought as more correlated than others
...
% Before discussing character correlation further,
Here we distinguish three different modes of forms of character correlation:
% MB: These considerations are interesting, and "modes" are also referred to in the title. The discussion does come back to these, but not sure it adds much. It could be an opportunity to remove almost a page and keep the overall flow more focused. Indeed, this is almost all said before (a bit more succinctly) by Mark Wilkinson.
...
\begin{itemize}
    ...
    Intra-organismal links can be the targets of comparative developmental biology \citep{goswami2010,kelly2010reduced,stoessel2013morphological,goswami2014macroevolutionary} or functional investigations. %AP - I would drop this sentence and say whether this type of correlation is useful or not in less uncertain terms.
    ...
    However, these methods do not give us a means to objectively control correlations that might adversely affect phylogenetic inference. %AP this is good.
    ...
    This coding dependency is linked to hierarchical dependency between characters \citep{wilkinson1995coping,BrazeauNA}. %AP-Can cut this sentence
...
Logical dependence, however, is easily distinguished prior to phylogenetic inference, while intra-organismal and evolutionary dependence are much harder to parse. %AP-what is 'logical dependece'?  its not defined... I guess you meaning "coding'... why is it easy to distinguish?
...
% MB: Well, software can maximise it, but compatibility itself is just a property of the data.
...
Therefore, phylogenetic character matrices that contain a great number of characters but a few number of tokens can easily lead to correlation between characters. %AP I do not understand this definition.  The example did not clear it up.  What are tokens?
% MB: I am having trouble interpreting this.
...
However, the other characters depict the same history %(i.e. the same split)
 but for the wrong reasons. % (note that even if \textbf{C2} was coded presence (1) or absence (0) of baleen, the same split would still be estimated by the software).
...
This example illustrates how some characters will therefore be expected to covary in a non-phylogenetic way, and that this phenomenon can reasonably be expected to mislead phylogenetic analysis. %AP-what is "covary in a non-phlyogenetic way'??  this section-I don't get it.
% MB: Is all of this necessary?

How do these specific correlation really affect topology?
Although it has been tackled empirically (for morphological data \citealt{Davalos01072014}, and molecular data \citealt{ZouConvergence}), the question has never been explored through a thorough simulation framework. %AP I hope we come back to these studies in the discussion.
% MB: You may wish to add one line to say what the fundamental importance of doing so is. What will this paper add that those did not?
...
\section{Methods}
To assess the effects of character correlation on the accuracy of phylogenetic inference we generated a series of matrices exhibiting different levels of correlation between some characters (Fig.\ref{Fig:outline} - note that each step is described in more details below): %AP different average correlation between characters?
\begin{enumerate}
...
%(i.e. entirely correlated characters would define identical splits between a set of taxa).
...
% Two same character states tokens have a difference of zero and two different ones have a difference of one
(e.g. $2 - 2 = 0$ or $1 - 8 = 1$).
% Additionally, we only consider differences for taxa with shared information \citep{GowerDist}.
...
When the character difference is maximal ($1$) it means the characters convey the greatest difference in phylogenetic signal. % (i.e. characters are uncorrelated).
%However, a character difference of $0$ %(i.e. the same phylogenetic signal)
%does not mean the opposite of $1$ (i.e. \textit{not} the opposite phylogenetic signal but the most different number of implied splits) . %AP this sentence was awkward or ungrammatical or incorrect, not sure which.  Don't talk about "opposites" it doesnt mean anything.
...
\begin{itemize}
    ...
    % This model has the advantage of not favouring Bayesian inference \citep[; see discussion]{OReilly20160081} but the downside of it is it can only generate binary state characters \citep[or 4 states;][]{puttick2017uncertain}.
...
% To estimate the variance of our simulations and assess the effect of our random parameters,
...
The maximum parsimony inference was run using a heuristic search with random sequence addition replicate 100 times with a limit of $5\times10^6$ rearrangements per replicates.
 % (\texttt{hsearch addseq=random nreps=100 rearrlimit=5000000 limitperrep=yes}).
...
Bayesian inference was run using an M\textit{k} model with ascertainment bias and four discrete gamma rate categories (M\textit{kv} $4\Gamma$)
 % - \texttt{lset nst=1 rates=gamma Ngammacat=4})
with an variable rate prior an exponential (0.5) shape.
 % (\texttt{prset ratepr=variable Shapepr=Exponential(0.5)}).
...
% \texttt{mcmc nruns=2 Nchains=6 ngen=1000000000 samplefreq=200 printfreq=2000 diagnfreq=10000 Stoprule=YES stopval=0.01 mcmcdiagn=YES}).
...
% However, we did report the comparison between the ``true'' trees and both the ``normal'' and ``randomised'' in the supplementary materials 3, Figs 15, 16 and 17.
...
The normalised score for both metrics thus reflects two distinct aspects of tree topology: (1) the Normalised Robinson-Foulds Similarity ($NTS_{RF}$)  reflects the conservation of clades
 % (i.e. a score close to $1$ indicates that most clades are identical in both trees)
; and (2) the Normalised Triplets Similarity ($NTS_{Tr}$) reflects the position of taxa.
% (i.e. a score close to $1$ indicates that most taxa have the same neighbours in both trees).
...
There is a significant difference between all scenarios in terms of clade conservation ($NTS_{RF}$ - Table \ref{Tab_pooledscenarios_test}).
 % (maximising, minimising and randomising; \textit{p} = 0; Table \ref{Tab_pooledscenarios_test}) with the highest probability of overlap being between minimising and randomising the character difference (BC = 0.954 for the $NTS_{RF}$; Table \ref{Tab_pooledscenarios_test}) and the lowest probability of overlap between maximising and minimising the character difference (BC = 0.848 for the $NTS_{RF}$; Table \ref{Tab_pooledscenarios_test}).
...
% , median $NTS_{RF} = 0.727$ and $NTS_{RF} = 0.865$ respectively; Wilcoxon text $p = 0$ and $BC = 0.914$ ).
...
% (median $NTS_{Tr}$ of 0.614, 0.793 and 0.896 for respectively 100, 350 and 1000 characters - supplementary materials 3, table 4) although the with a high distribution overlap between 350 and 1000 characters ($BC > 0.95$ - Table \ref{Tab_pooledscharacters_test}).
...
A low number of taxa (25) resulted in significant differences with both 75 or 150 taxa in $NTS_{RF}$ (Table \ref{Tab_pooledstaxa_test}) but no differences between 75 and 150 taxa for $NTS_{Tr}$ (Table \ref{Tab_pooledstaxa_test}).
 % (median $NTS_{RF}$ of 0.846 and 0.787 for 25 and 75 taxa respectively with a $p = 0$ and a $BC$ marginally $< 0.95$ - Table \ref{Tab_pooledstaxa_test} and supplementary materials 3, table 5) but no differences between 75 and 150 taxa and for $NTS_{Tr}$ ($BC > 0.95$ - Table \ref{Tab_pooledstaxa_test}).
...
\section{Discussion}
%AP- ok so im understanding that its important to measure CD because it tells you wether you are inflating your odds of getting a well resolved tree.  If you find low CD it means you should check your characters to see if you are committing coding dependency or if your traits are evolutionarily dependent then its probably an honest phylogenetic signal.  I'm not sure how Intra-organismal fits in though.  So basically the message of this paper should be "Always check your characters for correllation using this sweet metric-and if its high -check to see what kind of correlation you have"(This should go in the abstract as well) It'd then be nice to 'prescribe' methods for checking which kind of correlation you have.  Ok, Now I'm going to read the discussion and see if you did this.

\subsection{Effect of character differences on topology}%AP-the first sentence of the discussion should refer back to the title in my opinion.  Ie.  We find that different modes of character correlation have a strong effect on the ability to recover the correct phylogenetic tree.
...
When characters are correlated, matrices convey a strong (but potentially misleading) phylogenetic signal since characters are more likely to agree with each other and conversely, when characters are uncorrelated, one could expect them to convey a weaker phylogenetic signal with a high amount of homoplasy. %AP do you have to use 'homoplasy' here?  I had to look it up (for the billionth time) understand that it means independent trait origins- but dont understand how homoplasy would be related to weaker signal. Nor why a maximized tree would have higher homoplasy.
...
%AP this paragraph is so dry and boring, refer to the supplements??-get out of the weeds!  this is just rehashing results with no interpretation.  I suggest rewriting completely.
...
Overall, it is expected that minimising character difference %(increasing character correlation)
 is likely to have a greater effect on recovering the ``correct'' topology since most characters will provide the same phylogenetic signal. %ap- I would change "correct" to "more resolved"
...
%AP-The above paragraph isnt about characters or taxa, however it would be good for the first paragraph of the discussion.
While matrices with more characters or taxa tended to be better resolved than matrices with fewer characters or taxa, the effect of character correlation on topology is independent of the size of the matrix. (Tables \ref{Tab_pooledscharacters_test} and \ref{Tab_pooledstaxa_test}), %in terms of number of characters, between 100 and 1000 characters; 
...
%AP- the Bhattacharrya stuff doesnt make sense, do you mean BC is always high between matrices of different sizes? Why does it matter
...
Regarding the number of taxa, our results suggest the effect of character correlation on topology is independent on the size of the matrix when pooling the data. %ap Cut-as I summarized above
Furthermore, using the Normalised Tree Similarity metric ($NTS$) accounts for the fact that topological difference is affected by the sheer number of taxa considered.% (i.e. we corrected for the expected difference when comparing two random trees with the same number of taxa). %ap-This seems like weeds
...
% both using a Wilcoxon test and the Bhattacharrya Coefficient; 
...
It was however not the purpose of this study to compare the overall performance of both methods but rather to measure the effect of character correlation on each of those methods separately. %AP-Solid paragraph
...
% This can be also supported by the fact that Bayesian inference recovered the ``true'' tree topology slightly but consistently better (supplementary materials 3, figures 15, 16, and 17).
...
These two types of correlations can only be studied \textit{a posteriori} with a phylogenetic hypothesis and should not be used \textit{a priori} as a criterion to select characters. %AP-It'd be great to talk here about the problem with a priori selection creating circular and biased reasoning in a bit of depth.
...
%MB: These are the same types of correlation you mention in the preceding paragraph.
...
and (2) intra-organismal correlation is present in the matrices for the characters randomly simulated but sharing similar evolutionary simulation regimes.% (i.e. creating ``modules'' of characters).
However, the effect of these sources of correlation was out of the scope of this study and would have required \textit{a posteriori} changes to the matrices \citep{Lande1983,Maddison1990,Pagel1994}. %AP We should probably change the title of the paper then...
%(i.e. inferring the topology first and looking at character's correlation based on the observed topology \citealt{Lande1983,Maddison1990,Pagel1994}).
...
Second, modifying character correlation is difficult. %AP- the whole point is to start using CD to measure it...  you could maybe drop this paragraph and subsume the literature discussion into the discussion I suggested above about the 3 types of dependency and the potential for circular logic.  But maybe add a few lines on what you believe is state of the art for dealing with it, and then tie it up with a CHARGE to find betters ways to deal with it like in your monotreme study
...
% although aiming (and often achieving) to infer evolutionary history,
...
\subsection{Potential applications} %AP -yup  nice- maybe move above of caveats
...
%(i.e. the equivalent of randomising the character differences in our analysis)
will be sufficient to obtain a robust topology.% (\textit{cf.} actively collecting different characters).
...
%MB: Rewrite. Avoid informal constructions like "kind of...". As a side note: "kind of..." is a harmless thing to say, but after some things it can mean everything, like after "you're going to live" or "It's a boy!".
...
However, with
%the regain of popularity in discrete morphological data and
...
Moreover, because phylogenetic inference software are unable to \textit{a priori} differentiate between these correlation types, it is important to understand to what extent topologies can be induced %MB: Not sure this is word we want.
 by such bias.
...
Conversely, when correlation between characters was high, the topology was always the closest %MB: closer than what?
...
We recommend that all morphological phylogenetic analyses should report the level of character difference so that we can begin to detect and address the problem of coding dependence on inflating our confidence in our phylogenetic reconstructions. %Or some similar charge
...
% MB: I think the conclusion could be briefer: focus more on the broad-level implications and take it back to what this means for palaeontologists.

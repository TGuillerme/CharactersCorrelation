...
\begin{itemize}
    \item \textbf{Characters compatibility}: this is the result of how software actually interprets the differences between characters.
...
Furthermore, using simulations will allow drawing generalities on the effect of character correlation on phylogenetic inference and conversely to potential idiosyncrasies of empirical datasets.
...
We found an effect of the number of characters on the ability to recover the ``best'' topology with different levels of character correlations only between the two extreme number of characters (100 and 100; Tables \ref{Tab_pooledscharacters_test}).
This is expected since a higher number of characters can results in better topologies with a higher probability of ``noisy'' signal to cancel out (\textit{c.f.} in smaller matrices).
...
This can be explain partly because of our method to compare trees using the Normalised Tree Similarity \citep[$NTS$;][]{Bogdanowicz2012} which corrects for the fact that topological differences are proportional to the number of taxa considered.
...
Applying this type of bootstraps that minimised character difference by biasing the random sampling could thus help understanding which characters generate more homoplasy, which nodes in the tree are conflictual or which clades might be most dependent on highly correlated characters. 
Overall, we suggest measuring (and reporting) the levels of character difference in a matrix to help understand potential caveats in the distribution of optimal trees (e.g. a low average character difference will expect to generate more robust trees in bigger matrices).
%@@@ MB: I think we need to be careful here. Trees with a lot of high, consistent signal might appear to have low CD. What might be a better metric to investigate is the density of characters that have very high (like, perfectly correlated) CDs a hint that a lot of the data might reflect coding the same thing.
...
% @@@AP- the whole point is to start using CD to measure it...  you could maybe drop this paragraph and subsume the literature discussion into the discussion I suggested above about the 3 types of dependency and the potential for circular logic.  But maybe add a few lines on what you believe is state of the art for dealing with it, and then tie it up with a CHARGE to find betters ways to deal with it like in your monotreme study
...
We found that character correlation has a strong effect on recovering the correct topology with trees having high character differences always performing more poorly at recovering the correct topology than trees with low or randomised character differences.
%@@@ MB: Might be worth pointing to this and adding this as a reassurance: correlations not driven by selection  (e.g. pleitropy) could cancel each other out if they are randomly distributed  with respect to traits.
This is especially true in large matrices where we advice to reduce character difference by using bootstrapping with ``bias'' whereas this may not be major problem in modest size matrices (25 taxa; 100 to 350 characters).

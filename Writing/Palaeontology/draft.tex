...
\noindent {\normalsize \sc Thomas Guillerme$^{1,2,*}$, Abigail I. Pastore$^{2}$, and Martin D. Brazeau$^{3,4}$}\\
% Abigail I. Pastore$^{1}$
\noindent {\small \it 
...
$^3$Imperial College London, Silwood Park Campus, Department of Life Sciences, Buckhurst Road, Ascot SL5 7PY, United Kingdom.\\
$^4$Department of Earth Sciences, Natural History Museum, Cromwell Road, London, SW75BD, United Kingdom.\\}
...
However, encouragingly, they also suggest that, unless correlation is actively maximised or minimised, probabilistic methods can easily accommodate for a \textcolor{blue}{randomised} correlation between characters.
...
%% MDB: I think we can just start here. The paragraph above is completely extraneous for readers of Palaeontology.
The assumption of character independence is central to phylogenetic inference \citep[e.g.][]{joysey1982problems,felsenstein1985phylogenies,lewisa2001,felsenstein2004inferring}.
Contemporary methods like parsimony and maximum likelihood evaluate phylogenetic trees under the assumption that the input characters represent independent and identically distributed data points.
However, there are many cases in which morphological data can be reasonably suspected to have evolved in a non-independent way \cite[e.g.][]{Davalos01072014, ZouConvergence}.
Morphological character inter-dependencies can occur as a result of factors such as functional integration, underlying genetic modularity, or a  hierarchical relationship where one character depends directly on the state of another.
These relationships can create suites of similar character distributions that may bias phylogenetic analysis and deliver spurious results whose problems cannot be easily detected. 
At the same time, it is usually impossible to know whether similarities are homologous or the result of convergent evolution without prior knowledge of phylogeny itself.
This creates an impasse for morphologists, especially for the phylogenies of extinct groups where there is usually no likelihood of recovering molecular sequence data.

In this paper, we explore the circumstances under which character correlation might be expected to be a problem. Despite the realistic \textcolor{blue}{hurdles} posed by character correlations, it remains unclear that can be expected to be a pervasive problem. It is therefore worthwhile to characterise different types of character correlations that can impact comparative data. Here we distinguish four different modes of forms of character correlation (described in more details in \citealt{wilkinson1992ordered,wilkinson1995comparison,wilkinson1995coping}):

\begin{itemize}

    \item \textcolor{blue}{\textbf{Intra-organismal dependence:}} this is the result of a biological link between two characters that exists within the organism itself. The main cause of this would be pleiotropic genetic effects. This kind of dependence is often described in terms of integration and modularity, which can range from genetic to functional dependencies.
 
    \item \textbf{Evolutionary dependence:} this is the result of sets of characters co-evolving. 
    For example, in vertebrates, axial elongation can be correlated to limb reduction with snake-like bodies evolving multiple times in numerous tetrapod lineages. This type of dependence can be thought of in terms of functional suites of characters coevolving.
    ...
    \textcolor{blue}{\item \textbf{Phenomenological dependence:} This occurs where two morphological characters are expressed (often semantically) as two distinct things, but refer ultimately to the same phenomenon within an organism. For instance, occlusal patterns of mammalian dentitions would be expected to co-vary precisely in opposing mammal dentitions \cite{sansom2017dental}.
    A similar example is the relative position of the mandible in early gnathostomes and the course of its innervation. As the mandibular arch moves position relative to the rest of the braincase, the innervation patterns are expected to follow suit. Treating these as separate character may create spurious support for placoderm paraphyly \citep[e.g.][]{brazeau2009braincase,king2017bayesian}.}
    
\end{itemize}

\noindent Of course, the four sources of dependence are not necessarily exclusive: characters describing the left and right lower/upper molars will have \textcolor{blue}{phenomenological} dependence due to the modularity of the molars, their shared history and the duplicated coding.
...
It is also worth noting that the evolutionary and intra-organismal dependences induced correlations are by no means restricted to the use of discrete morphological data. In fact, by essence, these correlations appear to any type of data whether continuous (e.g. linear measurements or morphometric geometric measurements) or non-morphological (e.g. behaviour, or - much more commonly used - molecular data, \citealt{sansom2017dental}).}
...
\textcolor{blue}{We used the Mk model with ascertainment bias (Mkv) to correct for the fact that no invariant characters were present in the matrix (as is often the case with morphological data) allowing the model to estimate the correct evolutionary rates (see above).

...
(i.e. ``concentrated'' correlations) \textit{versus} weaker and more ``spread-out'' correlations in molecular data \citep[i.e. ``diffused'' correlations - though some regions of the genome may be thought as more correlated than others][]{huelsenbeck1999effect}.
...
Furthermore, it is important to note that on average, fewer characters were duplicated (replaced) in the ``minimised'' matrices compared to both the ``maximised'' and ``randomised'' matrices (supplementary material 3, Figs 7, 8 and 9 - likely due to the fact that replacing only a few highly different characters can drastically decrease the character difference).
% Add mention to this difference in character duplicates likely to affect the results

...
\noindent{\Large \bf Effect of discrete morphological character independence on tree topology}
\bigskip
...
\newpage
\begin{abstract}
- When using discrete morphological characters for building phylogenies, the characters are assumed to be independent.

- However, this assumption is nearly always wrong, simply because of the nature of morphological integration in organisms.

- The dependence between characters can originate from biological features, shared phylogenetic history or errors from the researcher when coding characters.

- Although the two first sources can be investigated by biologists post phylogenetic inference and the third one can be avoided pre inference with good practices, software do not distinguish between any of them.


- In this study, we propose a new metric of raw character difference as a proxy for character (in)dependence.

- Using thorough simulations and this metric, we test the effect of increasing or decreasing Character Difference on topology with a variable number of taxa and characters under both Bayesian and Maximum Parsimony inference.
...
Because both $NTS_{RF}$ and $NTS_{Tr}$ metrics are bounded at one, the resulting in the residuals of any model based on the $NTS$ scores were not normal thus preventing the use of parametric tests for comparisons (see Supplementary @@@). %TG: TODO: residual normality + homscedasticity tests supp.
...
%TG: @@@ NOTE: THESE RESULTS ARE NOT FINAL - I thus didn't spend too much time describing them @@@
...
We found that Character Differences as a proxy for character correlation have a strong effect on the recovering the ``normal'' topology

 is not a problem \textit{per se} as long as characters are not actively coded to be similar.

...
\end{equation}
%Why divide by 1/2 when you can just multiply by 2?
...
%MB: Splits rather than groups, really. Since, in a rooted tree, at most one symbol will need to define a clade. All others could be plesiomorphic states.
...
%MB: Part of it is: we expect it to be a problem, but _when_ do we expect it to be a problem. There's the issue of "check your data", but it only raises the question: how? I think this is where we suggest avenues for future research (either here or move it up to the discussion). For instance, the simulations could be run in a way that more 'intelligently' places correlations in different parts of the tree. So, correlations are specifically modeled in an underlying stochastic process, for instance. We also don't really explore character congruence, that correlations can be ranked and spread over multiple internodes. We didn't look at ranking and we didn't look at competing correlations. There's a lot more that could be done here so, we might talk about that. Then we could talk about intelligent resampling measures, such as a targeted jackknifing that preferentially resamples characters that are highly similar to other characters etc. 

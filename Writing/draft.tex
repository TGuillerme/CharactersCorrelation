...
%     \begin{aligned}
    ...
    % \abs*{\frac{\sum_{i}^{n}\abs{x_{i}-y_{i}}}{n}-\frac{1}{2}}}} 
    %  &\geq 
    %  \abs*{\frac{\sum_{k}^{j}\abs{x_{k}-y_{k}}}{j}-\frac{1}{2}}}} 
    %  +
    %  \abs*{\frac{\sum_{a}^{b}\abs{x_{a}-y_{a}}}{b}-\frac{1}{2}}}} - 2\\

     % Add brackets symbols

     % \in[0,\frac{1}{2}] &\geq (-2) \in[0,\frac{1}{2}] \in[0,\frac{1}{2}]\\
...
% \subsubsection{Character difference metric is ontological (identity of indiscernibles)}

% \begin{equation}
%     \label{CDnull}
%     CD_{(x,y)} = 0 \Leftrightarrow x = y
% \end{equation}
% This is because of equation \ref{equality}; in fact $x$ and $y$ is actually expressed in terms of phylogenetic information per character (i.e. normalised for character state token). 


% \subsubsection{Character difference metric is sub-additive (triangle inequality)}

% \begin{equation}
%     \label{subadditive}
%     CD_{(x,y)} \leq CD_{(x,z)} + CD_{(z,y)}
% \end{equation}
% Following equations \ref{CDpositive}, \ref{equality}, \ref{CDnull}.

% Thus:

% \begin{equation}
%     d: \frac{\abs*{\sum_{i}^{n}(x_{i} - y_{i})}{n} \rightarrow [0;1]
% \end{equation}

% Thus:

% \begin{equation}
%     d: \frac{\sum_{i}^{n}(x_{i} - y_{i})}{n} \rightarrow [0;1]
% \end{equation}

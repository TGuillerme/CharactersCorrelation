...
\begin{itemize}
    ...
    %MB: We should probably cite some "Gunterology" here.
...
\item \textbf{Inferring topologies}: we inferred the topologies from the ``normal'', ``maximised'', ``minimised'' and ``randomised'' matrices using both maximum parsimony % (MP) %TG: not a big fan of the abbreviations - however if you think they're necessary, we can always add them.
 and Bayesian inference.% (BPP). 
...
%TG: OK, I've checked Farris' stuff. Despite not beeing described (at least not the in 1970 Syst Biol paper), it seems that the advancement index is just the distance between two taxa. Appart from the taxa/character difference, it's kind of similar but (1) it's not ponderated (divided by n), centered (-1/2) and reversed (1-2(...)); so I'm not sure if we need to say it's similar. In the end it's a distance metric and it can be pretty similar to *any* distance metric. I think we can leave as it is (plus, I've reinvented the wheel by mathematicallty demonstrating it's a distance - my first only real math in a draft).

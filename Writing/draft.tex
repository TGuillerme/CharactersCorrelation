...
% I'm unsure what point this introduction serves. It somehow doesn't quite connect with what you say next. I think we can simply focus on the fact that the phenomenon hasn't been empirically explored. 
% I think the point in this manuscript should be: OK, we believe two reasonable things: some characters will be expected to covary in a non-phylogenetic way, and that this phenomenon can reasonably be expected to mislead phylogenetic analysis. 
% The question then becomes: how bad does it have to be to be a real effect?
% Along with this are subquestions:
% If you've got intrinsic correlations, then those correlations could occur across anatomical systems. Would their effects cancel each other out?
% Will "noisier" characters accurately cancel out the effects of correlation?
...
To assess the effects of character correlation on the accuracy of phylogenetic inference we generated a series of matrices exhibiting different levels of correlation between some characters (Fig.\ref{Fig:outline}):
\begin{enumerate}
...
% HOW do you simulate them? What models do you use? What protocol? Are the simulated matrices expected to recover the correct topology in the absence of ourmanipulations? 
...
% I don't understand this. Would a matrix with a "maximised" level of correlation simply be a uniform matrix?
% What do you mean by "least" and "most different"? What is the measure of similarity? 
% What do you mean "replacing them randomly by the remaining characters? Also: won't that simply create a correlation with another character in the dataset?
...
% Are you sure this is new? I can't read the formula because it's in LaTeX gobbledygook but there exist methods for measuring character similarity,
% I can't be sure, but this could as well be Farris's (1970) advancement index as applied to characters rather than taxa?
...
% What? Also, do you mean the taxon i? (not "taxa i").
...
% I don't get this part. What do you mean by standardized? And why are you doing this? Are you just editing the symbols?
...
% If we are using undirected characters, does it matter what the rooting is?
These trees are hereafter called the ``true'' trees.
% Yay! The Truth! Stop sciencing!
...
\begin{itemize}
    ...
    %TG: is that not a random Dirichlet? D(A,G,C,T)?
...
% I'm not going to change all these, but "maximum parsimony" isn't a proper noun, so I wouldn't capitalise it.
...
%"the resulting in residuals..." I'm not sure what this sentence is supposed to say, so I can't suggest an edit.
...
% Here and other similar subsection headings: the effect on what?
...
% The above sentence has too many "for"s. It's too long and needs to be split to be intelligible.
...
% Do we? I'm not sure that I would. In practice, this would probably force morphologists to remove most of their characters.
% Making these kinds of recommendations leads to the risk that all you do is introduce biases into the dataset in another way.
...
% This is serious hedging ("we're not going to... but we're going to"). Could probably skip the above line.
...
% This doesn't make sense. Longer than one? How is generating longer trees coping? What do you mean by "coping"? 
...
% I'm not sure I'm convinced by this paragraph. Perhaps we should have a chat about it.

\subsection{Distinction between different character correlations}
The two types of correlations that most evolutionary biologists are genuinely interested in (intra-organismal and evolutionary) can only be studied \textit{a posteriori} using phylogenetic trees and should not be used as an \textit{a priori} criterion to validate/invalidate characters.
% Not sure I follow this line. Maybe there's another way to phrase it.
...
% This basically means choosing the dataset with the highest homoplasy... which will, in the long run, fuck up any method.
% From a structural point of view, this line is out of place even. It makes a general point, but deep in the subsection. Such statements are better made earlier so a reader knows what the point of the section is.
Evolutionary correlation is also present in this simulations since the characters are modelled on Birth-Death trees.
Similarly, intra-organismal induced correlation (i.e. modularity) can be present in the matrices for those characters randomly simulated with similar evolutionary simulation regimes.
...
Correlation between characters can be induced through three main phenomena: intra-organismal relationships, selection-driven covariation or biases in coding the characters.
Although one could argue that coding-induced correlation is easily avoidable and that intra-organismal and evolutionary correlations are actually at the core of evolutionary biology, in the age of ever expanding morphological matrices \citep[e.g.][with more than 1000 characters each]{nithe2013,O'Leary08022013}, we can expect the dependence between characters to increase stochastically.
% I have no idea what thae above sentence means.
...
% But at what point? How much correlation is needed to affect it?
...
% This line sounds like something from an undergraduate term paper.

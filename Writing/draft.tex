...
\begin{itemize}
%TODO: Change biological throughout
    ...
    These links can be the targets of comparative developmental biology \citep{goswami2006morphological,goswami2010,goswami2014macroevolutionary}.


%TODO: add somewhere the problem that we use model organisms for comparative developmental bio which is shit. We may never be in a realistic position to actually know these interactions.
...
%TG: Yep, totally synapormophies. Although there is a subtle nuance in the way they are studied. In pure phylo approach (i.e. matrix -> tree), the synapomorphic characters are participating to the topology inference and are thus impossible to distinguish (i.e. the synapomorphies make the trees and the trees make the synapomorphies). In the papers cited just below they have a different approach. They assume a phylogeny and then "map" characters on to it. In this later case, the synapomorphies are effectively decoupled from the topology.
...
%TG: good call, I've changed the example.
    ...
    %TODO: hierachical (logical dependency) problem (Wilkinson's papers 93-95 - Inapplicability).
    %atomization: necessary evil; dataset size needed to overcome missing data
    %biased density: e.g. skull: because they're complex, but localise information so high likelihood of type-1 correlation (cf a finite state space)
    %                     things with high preservation potential (a finite state space, but also functional auto-correlation because the cause of high preservation potential can be e.g. biomechanical)
\end{itemize}
...
Logical dependence, however, is easily distinguished prior to phylogenetic inference, while the two other ones (biological and evolutionary) are much harder.
...
%What the softwares are confronted with. Two dimensional matrix problem (renders the subtilties opac). The correlations are not distinguishable anymore.

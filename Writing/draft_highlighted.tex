...
However, this effect is less important for matrices with a small number of taxa (25 in our simulations) where \hl{reducing or increasing} character correlation is not more effective than randomly drawing characters.
...
\hl{).
Second, this discrepancy has been suggested to be stronger correlations in morphological data (i.e. ``concentrated'' correlations) \textit{versus} weaker and more ``spread'' correlations in molecular data}
...
\hl{However, we did report the comparison between the ``true'' trees and ``normal'' and ``randomised'' ones is available in the supplementary materials 3, Figs 12, 13 and 14.}

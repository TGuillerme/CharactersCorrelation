...
\end{equation}
%This is not making sense to me right now... I'll ask you about it when you get home. Nope I got it now.
...
%UG FRUSTRATING -A LOT OF MY EDITS HAVE GOTTEN DELETED :(   OH WELL I'LL JUST PUT BACK THE SUPER IMPORTANT ONES. x
%soooo CD_AC = (0+0+1+1)/(4-1) =2/3  right now.   C needs to change to C={0,0,2,3}  then you get (0+1+1+1)/3=1.  I suggest putting the rough calculation in like I demonstrated-because it's a little bit confusing to figure out.
...
%I can't find Table 2 in this document...

%I strongly suggest that you labelled these figures more on the fig-put less in the legend-give them titles of Robinson Ford or triplet whatever.  Then in the first line of the fig text- you can say "Robinson Ford distance shows the degree of clade conservatism whatever that means... "  so the reader can immediately know the difference between fig 2 and 3.  To label: (1) text on ylab "Normalized tree similarity"- then each row can just be "# taxa" (2) mtext on xlab "Number of characters"  (3)  legend c("more correlated", "less correlated", "randomised") but leave bayesian and parsimony for the fig text.  The goal being to know what the figure says at a glance-the fig text can help clarify
\begin{figure}[!htbp]
\centering
...
%TODO: [discussion] This can be explained by lower resolution for Bayesian
...
However, the effect of these sources of correlation was out of the scope of this study and would have required \textit{a posteriori} changes to the matrices.% which are - when using empirical data - at best bad practice and at worst dishonest. I DONT  REALLY UNDERSTAND

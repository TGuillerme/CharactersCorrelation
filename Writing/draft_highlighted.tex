...
% \hl{Furthermore, phenotypic variations can be continuous rather than discrete and ``discretising'' such characters can have some influence on tree inference} \citep{parins2017use}.
% \hl{In this case, ``discretised'' }.
...
%TG: Modifications in this sentence not in the letter yet
...
\hl{There is however no difference in terms of taxa displacement ($NTS_{Tr}$) between the three scenarios (with a BC > 0.98 for each comparison; Table \ref{Tab_pooledscenarios_test}).}

\input{Tables/Tab_pooledscenarios_test.tex}

\subsubsection{Number of characters}
\hl{This effect of the character difference affects clade conservation ($NTS_{RF}$) only by big differences in number of characters (100 \textit{vs.} 1000, median $NTS_{RF} = 0.727$ and $NTS_{RF} = 0.865$ respectively; Wilcoxon text $p = 0$ and $BC = 0.914$ \ref{Tab_pooledscharacters_test}).}
\hl{For closer differences of characters (i.e. 100 \textit{vs.} 350 and 350 \textit{vs.} 1000), Wilcoxon tests where different ($p = 0$) but with high distribution overlap ($BC > 0.96$).}


is not dependent on the number of characters when looking at clade conservation (i.e. $NTS_{RF}$).


The median $NTS_{RF}$ was similar for 100, 350 and 1000 characters (0.730, 0.745, 0.767 respectively; see supplementary materials 3 \hl{Table 3}) with a significant difference only between 100 and 1000 and 350 and 1000 characters (\hl{\textit{p} = 0.001 and \textit{p} = 0.032 respectively}; Table \ref{Tab_pooledscharacters_test}).
The number of characters affects the character difference more in terms of taxon placement for a low number of characters (median $NTS_{Tr}$ for 100, 350 and 1000 characters equals 0.544, 0.693, 0.799 respectively - see supplementary materials 3 \hl{Table 3}) with a significant difference between 100 and 350 or 1000 characters (\hl{\textit{p} = 0, \textit{p} = 0 and \textit{p} = 0.014 respectively}; Table \ref{Tab_pooledscharacters_test}).
...
Furthermore, the trees generated by the ``minimised'' scenario do not appear better resolved (towards any topology) than the other scenarios (see supplementary material 3, \hl{Figs 6, 7 and 8}).
...
Calculations where done using the Imperial College London Cluster Services (doi: 10.14469/hpc/2232). @@@ plus awoonga
We thank Alberto Pascual Garcia for input in the design of the simulations protocol, Guillermo Herraiz Yebes and Abigail Pastore for helping with the CD metric proof and Orlin Todorov for assistance with the computations. We also thank the \hl{three anonymous reviewers for there constructive and insightful comments}.

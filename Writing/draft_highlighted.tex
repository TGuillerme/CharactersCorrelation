...
For example with three characters \texttt{A = \{0,1,1,1\}}, \texttt{B = \{1,0,0,0\}} and \texttt{C = \{0,0,1,2\}}, $CD_{(A,B)} = 0$ and $CD_{(A,C)} = 1$ \hl{$CD_{(A,B)} = (0+0+0+0)/(4-1) = 0$ and $CD_{(A,C)} = (0+1+1+1)/(4-1) = 1$}.
...
%TODO: I strongly suggest that you labelled these figures more on the fig-put less in the legend-give them titles of Robinson Ford or triplet whatever.  Then in the first line of the fig text- you can say "Robinson Ford distance shows the degree of clade conservatism whatever that means... "  so the reader can immediately know the difference between fig 2 and 3.  To label: (1) text on ylab "Normalized tree similarity"- then each row can just be "# taxa" (2) mtext on xlab "Number of characters"  (3)  legend c("more correlated", "less correlated", "randomised") but leave bayesian and parsimony for the fig text.  The goal being to know what the figure says at a glance-the fig text can help clarify
...
\ref{Tab_pooledscenarios_test}).  %AB: TODO: HERE  CAN YOU GIVE A LITTLE MORE INTUITION ON WHAT THE RESULTS MEAN

%TODO: maybe this is table 2?  You say "differennce between the pooled scenarios"  I don't remember what you are talking about-maybe describe the pooled scenarios for the forgetful reader.  I have no idea what table 2 is even saying-the table text could use more hand holding explanations.  # same issues with table 3.  I think I personally would've done these stats differently as a multilevel model so that the bonferroni corrections weren't necessary-but I suppose it doesnt fucking matter with p values like that.  However, table 4 is perfect: don't change a thing.
...
\hl{When comparing the distribution using the Wilcoxon ranked tests, there was no clear differences  between  tree similarity of differening numbers of taxa, characters, tree correlation, or tree inference method for the taxa displacement metric ($NTS_{Tr}$ - supplementary material 3, tables 21 and 22).}
...
%AB: This is only true for the RF method, no?
...
Intuitively, one would hypothesise that the ``minimised'' character difference scenario to lead to incorrect but consistent trees, the ``maximised'' scenario to lead to poorly resolved once (very homoplasic trees) and the ``randomised'' scenario to perform the best at recovering the correct topology. 
%AB: TODO: I DONT SEE ANY DATA THAT SUPPORTS THIS STATEMENT
...
\hl{This
%AB: TODO (WHAT IS THIS?,  CHARACTER CORRELATION?)
did not however appear to affect the taxa placement ($NTS_{Tr}$) for which the confidence intervals overlapped too much to be able to discern clear statistical differences (Fig.}
...
\hl{Furthermore, it is important to note that on average, less characters where duplicated (replaced) in the ``minimised'' matrices compared to both the ``maximised'' and ``randomised'' matrices (supplementary material 3, Figs 4, 5 and 6).
%AB: TODO:  WHY DID THIS HAPPEN?WHY IS IT IMPORTANT ? 
However,
%AB: TODO (THIS CLAUSE IS CONFUSING AND KIND OF SOUNDS LIKE BULLSHIT )
this change in the amount of replaced characters did not change the way average character differences varyed across the three transformed matrices: the character differences were always consistently increased (``maximised''), decreased (``minimised'') and maintained at a normal level (``randomised'') for each replicate (supplementary material 3, Figs 1, 2 and 3).
...
%AB: TODO: I SUDDENLY REALLY DONT LIKE THE TERM CHARACTER DIFFERENCES-ITS THE OPPOSITE OF CHARACTER CORRALTION? IT DOESNT SOUND LIKE IT INTUITIVELY. ITS KIND OF CONFUSING TO HAVE SO MANY TERMS MEANING THE SAME THING -MAXIMIZED V MINIMIZED, CHARACTER CORRALATION, CHARACTER DIFFERENCE.  I'M TIRED AND GRUMPY.
...
%AB: TODO: DID YOU TALK ABOUT POOLING DATA IN THE METHOSA? I'M NOT SURE WHAT YOU ARE REFERING TO.
...
%TODO: Bayesian recovers the ``true'' tree topology slighltly but consistently better (supplementary materials 3, figures 12, 13, and 14).
...
However, the effect of these sources of correlation was out of the scope of this study and would have required \textit{a posteriori} changes to the matrices.% which are - when using empirical data - at best bad practice and at worst dishonest. AB: TODO: I DONT  REALLY UNDERSTAND
...
%AB: TODO: HMM IS THIS CIRCULAR LOGIC?  YOU KNOW THAT CHARACTERS ARE ON AVERAGE INFOMRATIVE-SO OF COURSE INCREASING THE REPRESENTATION OF INFROMATIVE CHARACTERS INCREASES YOUR ABILITY TO CORRELTY RSOLVE.  IF CHRACTERS WERE UNIFORMATIVE AND CORRELATED THIS RESULT WOULD NOT HOLD
...
%AB: TODO: (DID YOU LOOK AT WHICH TOPOLOGIES WERE INDUCED??)
